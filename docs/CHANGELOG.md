...
### Changed
- **`mulmoclaude@0.8.1`** — adds `schemaDocs` / `getSchema` / `putSchema` actions to the `manageCollection` MCP tool so Claude reads the collection-authoring reference and validates-then-writes a collection's `schema.json` instead of raw, unvalidated file edits; `putSchema` runs discovery's post-Zod acceptance gates host-side (built on the already-published `resolveDataDir`) so a write can't pass yet vanish on the next discovery. Picks up the three shared-package bumps below via its existing caret ranges (pin the minimums at app-publish time, once they're on the registry).
