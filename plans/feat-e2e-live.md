...
| **L-33** mc-cooking-coach preset chain canary (#1287) | ✅ 実装済 | skills.spec.ts、 launcher 同梱の `mc-cooking-coach` を「`server/workspace/skills-preset/<slug>/SKILL.md` → `data/skills/catalog/preset/<slug>/` → `.claude/skills/<slug>/` → `/<slug>` slash dispatch → 一ターン目で cooking 関連 vocabulary を含む応答」 の 4 段 chain で end-to-end 検証 (B 重要度)。 `/skills` 直叩き → `skill-catalog-item-mc-cooking-coach` visible で `syncPresetSkills` 出力を確認 → `snapshotProjectSkillSlugs()` で 既 star 済なら skip、 未 star なら catalog detail の `skill-catalog-detail-star-btn` を click して active 化 (preset は launcher 管理なので unstar はしない、 idempotent setup) → `skill-item-<slug>` click → `skill-body-rendered` に signature line `"bundled MulmoClaude preset skill"` が含まれることを assert (catalog→active 経路で本文が mangled していないこと) → `skill-run-btn` click → `/chat/<id>` で assistant 応答が `/recipe|cook|料理|レシピ/i` のいずれかにマッチすることを assert (body が agent context に乗ったか の signal)。 L-22 が直 seed→Run、 L-32 が agent-authored 経由を見るのに対し、 L-33 は **launcher tarball 起点の preset adoption rail** を独立して見る canary。 fake-echo は slash dispatch + skill resolver を fake 不能なので per-test に `E2E_LIVE_NO_LLM=1` skip |
